...
const price = PLANS[selectedPlan].price;
  ...
  return (
    <div className="bg-price-white dark:bg-black rounded z-10 divide-y mt-44 sm:mt-8 sm:m-auto">
      <div className="p-7 flex flex-col gap-8">
        <div className="grid gap-5 sm:grid-cols-2 sm:gap-8">
        <h1 className="text-grayish-blue dark:text-white text-sm text-center py-4 font-sans font-semibold tracking-wider sm:mt-3 sm:text-left">
          ...
          <div className="text-center text-dark-desaturated-blue flex justify-center items-center  gap-1 font-bold pt-3">
            <span className="text-3xl sm:text-4xl dark:text-white">
              {isChecked ? `$${price * 0.75}.00 ` : `$${price}.00`}
            </span>
            <span className="text-grayish-blue dark:text-white text-sm">/month</span>
          </div>
        </div>
        <div className="flex gap-3 text-xs text-grayish-blue font-bold items-center justify-center">
          <span className="dark:text-white">Monthly Billing</span>
          <Toggle
            checked={isChecked}
            onClick={() => setIsChecked(!isChecked)}
          />
          <div className="flex items-center gap-1">
            <span className="dark:text-white">Yearly Billing</span>
            <span className="bg-light-grayish-red rounded-full text-[9px] text-light-red py-0.5 sm:py-1 px-2 sm:px-5">
              <span className="sm:hidden dark:text-white">-</span>
              25%
